...
from russkook_jial8_dbg_agent import DSBG
from russkook_jial8_sbg_agent import SBG
...
def run(agent1, agent2, max_secs_per_move, initial_state=bgstate(), deterministic=False):
  ...
  while(not DONE):
    ...
    #for normal input
      ...
      #iterating through the 2 checkers to move
      for i in range(2): # 0, 1... checker 0 and 1
        ...
        #pt represents points on board
        #i think this syntax means if i = 0 return int(checker1) and if i = 1 returns int(checker2)
        pt = int([checker1, checker2][i])
        
        # Check first for a move from the bar:
        if pt==0:#moving checker from bar
        ...
        if pt < 1 or pt > 24:
          ...
          forfeit(whose_move)
          break
        #why pt-1? it's 0 index array but on board it's 1 index
